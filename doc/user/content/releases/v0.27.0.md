...
* **Breaking change**. [PostgreSQL sources](/sql/create-source/postgres)
  materialize replicated upstream tables directly. You must specify which
  upstream tables to replicate during creation of the source using
  `FOR ALL TABLES` or `FOR TABLES (...)`.
